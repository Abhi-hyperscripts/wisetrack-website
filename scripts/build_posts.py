...
POSTS = [
    ...
    {
        "slug": "post-zoho-finance-deep",
        "title": "Zoho, but how a 50-person finance team would want it.",
        "italic_word": "want",
        "subtitle": "The patterns that show up over and over when we wire deep finance ops into Zoho.",
        "date_iso": "2025-06-17",
        "date_human": "17 Jun 2025",
        "author": "Karthik",
        "read": "7 min read",
        "tag": "Zoho",
        "tag_color": "tag-cyan",
        "summary": "The patterns that show up over and over when we wire deep finance ops into Zoho — and what we wish more teams knew before they started.",
        "toc": [
            ("why-zoho", "01 · Why teams pick Zoho"),
            ("the-three-gaps", "02 · The three gaps"),
            ("our-pattern", "03 · The pattern we use"),
            ("what-not-to-build", "04 · What not to build"),
        ],
        "sections": [
            ("why-zoho", "01", "Why teams pick Zoho — and where it stops.",
             [
                "Zoho One is unreasonably good value for a 30-to-150-person company. You get Books, CRM, Inventory, Sign, People, Desk, and twenty other things, all for less than what most companies pay for a single mid-market SaaS. That alone earns it a place in the stack.",
                "It stops being enough when your operations diverge from generic. A standard \"sales order → invoice → payment\" flow works beautifully out of the box. The moment you add anything specific — multi-warehouse stock reservation, partial fulfilment across vendors, custom approval matrices keyed off deal size, a regional tax rule Zoho's tax engine doesn't model — the cracks show.",
                "We do a lot of work in that crack. Not replacing Zoho. Extending it.",
             ]),
            ("the-three-gaps", "02", "The three gaps we keep filling.",
             [
                "<strong>Approval routing.</strong> Zoho's built-in approval flows handle simple chains. Real finance ops have approvals that branch on deal size, customer category, regional manager, and exception status. We build that routing engine outside Zoho, talk to it over the API, and write the result back as an audit trail.",
                "<strong>Cross-module logic.</strong> Books knows about invoices. Inventory knows about stock. CRM knows about deals. When a single business rule needs to span all three (\"if this deal closes, reserve stock from the closest warehouse and flag the customer's credit limit\"), Zoho can't express it cleanly. We build a thin orchestration service that owns the rule and calls into each module.",
                "<strong>Reporting beyond Books.</strong> Zoho's reports are fine for accountants. They are not fine for a CFO who wants a single live dashboard pulling Books, Inventory, and the bank feed at the same time. We build that dashboard separately, fed by a sync job that reads Zoho's APIs and writes to a reporting Postgres.",
             ]),
            ("our-pattern", "03", "The pattern we use.",
             [
                "Three principles. First: <strong>Zoho is the system of record</strong> for the data it natively owns — Books for ledger entries, CRM for the deal pipeline, Inventory for stock. We never duplicate that data anywhere it could go stale.",
                "Second: <strong>our service owns the workflow</strong>. The orchestration rules, the approval engine, the audit trail, the dashboard reporting — these live in a small ASP.NET Core service we run on the client's infrastructure. It calls Zoho's APIs to read and write authoritative state, but the business logic does not live inside Zoho's custom-function sandbox.",
                "Third: <strong>we treat the API as a contract</strong>. Zoho's APIs are reliable but they version and rate-limit. We wrap every endpoint call in a typed client, batch where we can, retry where it's safe, and surface failures clearly. The client never sees us hammering Zoho.",
             ]),
            ("what-not-to-build", "04", "What not to build.",
             [
                "There is a strong temptation, the first time you outgrow Zoho's defaults, to migrate off it entirely. Don't.",
                "Migrating off Zoho is expensive in the same way migrating off Postgres is expensive — paid for in engineering hours, operational complexity, and onboarding tax. And the alternative is rarely as cheap as the bundle you're leaving. Build around the gaps, not against them.",
                "We also don't build a duplicate of Zoho's UI for the finance team. They already know Zoho's interface. Adding our custom dashboards alongside is fine. Replacing the parts of Zoho that work well with a custom-built thing is, in our experience, a path to a worse finance team.",
             ]),
        ],
    },
    {
...
<div class="fixed top-0 left-0 right-0 z-[70] bg-bg/85 backdrop-blur-xl border-b border-white/5">
  <div class="max-w-page mx-auto px-edge h-9 flex items-center justify-between text-[11px] mono tracking-wider text-muted">
    <div class="flex items-center gap-3"><span class="dot"></span><span class="hidden sm:inline">JOURNAL · {date_human} · {read}</span><span class="sm:hidden">JOURNAL</span></div>
...
<header class="fixed top-9 left-0 right-0 z-[60] bg-bg/55 backdrop-blur-2xl border-b border-white/5">
  <nav class="max-w-page mx-auto px-edge h-[80px] flex items-center justify-between">
    ...
    <div class="hidden md:block">
      <div class="nav-links">
        ...
        <a href="insights.html" class="is-active">Journal</a>
  ...
  <div data-menu class="md:hidden hidden bg-bg border-t border-white/5 [&.open]:block">
    <div class="px-edge py-6 flex flex-col gap-4 text-lg">
      <a href="solutions.html" class="py-2">Solutions</a>
      <a href="portfolio.html" class="py-2">Work</a>
      <a href="methodology.html" class="py-2">Method</a>
      <a href="insights.html" class="py-2 text-text">Journal</a>
...
<section class="relative overflow-hidden">
  ...
  <div class="relative max-w-page mx-auto px-edge pt-12 md:pt-16 pb-16">
    <div class="relative hud hud-cyan p-6 md:p-10 lg:p-14 border border-white/8 rounded-2xl bg-white/[0.015]">
      ...
      <div class="mt-8">
        <a href="insights.html" class="inline-flex items-center gap-2 mono text-[11px] tracking-widest text-muted hover:text-text transition-colors">
...
<section class="relative border-t border-white/5">
  <div class="max-w-page mx-auto px-edge py-16 grid grid-cols-12 gap-gutter items-center">
    ...
    <div class="col-span-12 md:col-span-5 md:text-right flex flex-col md:items-end gap-3">
      <a href="insights.html" class="btn-ghost w-fit">All articles <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M3 7h8M7 3l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
